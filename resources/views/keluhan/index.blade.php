                            ...
                            <tbody>
                                @forelse($keluhan as $index => $item)
                                    <tr>
                                        ...
                                        <td>
                                            @if ($item->gambar)
                                                <img src="{{ asset('storage/' . $item->gambar) }}" alt="Gambar Keluhan"
                                                    width="80">
                                            @else
                                    ...
                                    : ($item->status == 'diproses'
                                        ...
                                        <td>
                                            <div class="d-flex flex-column gap-2">
                                                ...
                                                {{-- Tombol Lihat --}}
                                                <a href="{{ route('keluhan.show', $item->id) }}"
                                                ...
                                                @if (auth()->user()->role === 'admin' && $item->status == 'pending')
                                                    ...
                                                    <!-- Modal Tanggapi -->
                                                    <div class="modal fade" id="tanggapiModal{{ $item->id }}"
                                                        tabindex="-1" aria-labelledby="modalLabel{{ $item->id }}"
                                                        aria-hidden="true">
                                                        <div class="modal-dialog modal-dialog-centered">
                                                            <div class="modal-content">
                                                                <form action="{{ route('keluhan.tanggapi', $item->id) }}"
                                                                    method="POST">
                                                                    @csrf
                                                                    <div class="modal-header">
                                                                        <h5 class="modal-title"
                                                                            id="modalLabel{{ $item->id }}">Tanggapi
                                                                            Keluhan</h5>
                                                                    ...
                                                                    <div class="modal-body">
                                                                        <div class="form-group">
                                                                            <label
                                                                                for="respon_admin_{{ $item->id }}">Tanggapan</label>
                                                                            <textarea id="respon_admin_{{ $item->id }}" name="respon_admin" class="form-control" rows="4" required></textarea>
                                                                        </div>
                                                                    </div>
                                                                    <div class="modal-footer">
                                                                        ...
                                                                        <button type="submit"
                                                                            class="btn btn-primary">Tandai Diproses</button>
                                                                    </div>
                                                                </form>
                                                            </div>
                                                        </div>
                                                    ...
                                                    <div class="modal-dialog">
                                                        <div class="modal-content border-danger">
                                                            <div class="modal-header bg-danger text-white">
                                                                <h5 class="modal-title"
                                                                    id="hapusModalLabel{{ $item->id }}">Hapus Keluhan
                                                                </h5>
                                                                <button type="button" class="btn-close"
                                                                    data-bs-dismiss="modal" aria-label="Tutup"></button>
                                                            </div>
                                                            <div class="modal-body">
                                                                Yakin ingin menghapus keluhan
                                                                <strong>{{ $item->judul }}</strong>?
                                                            </div>
                                                            <div class="modal-footer">
                                                                <form action="{{ route('keluhan.destroy', $item->id) }}"
                                                                    method="POST">
                                                                    @csrf
                                                                    @method('DELETE')
